// ShellExt1.cpp : アプリケーションのエントリ ポイントを定義します。
//
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_SHELLEXT1, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_SHELLEXT1));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_SHELLEXT1));
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_SHELLEXT1);
...
typedef struct tagMYPIDLDATA {
    ...
    USHORT cb;
    // オブジェクトの種類を示す拡張定義値
    // フォルダーの場合は dwType が TRUE 、それ以外の場合は FALSE
    DWORD dwType;
    // オブジェクトの表示名
    // WSzDisplayName を 40 文字に設定して 、SHITEMID 構造体が DWORD にアラインされることを保証
...
/// <summary>
/// オブジェクトの型と表示名を受け取り、オブジェクトの単一レベルの PIDL を返します
/// </summary>
/// <param name="dwType"></param>
/// <param name="pwszDisplayName"></param>
/// <returns></returns>
LPITEMIDLIST CreatePIDL(DWORD dwType, LPCWSTR pwszDisplayName) {
    ...
    HRESULT hr = CoGetMalloc(1, &m_pMalloc);
    if (hr != S_OK) {
    ...
    if (pidlOut) {
        ...
        hr = StringCbCopyW(pidlOut->wszDisplayName, sizeof(pidlOut->wszDisplayName), pwszDisplayName);
